...
<!DOCTYPE html>
	<html lang="en-us">
		...
		<body>
			...
			<br>
			<p class="boxtextright"><u><a href="<?php echo base_url().'/History' ?>" target="_blank">ประวัติรายการสร้าง Short URL</a></u></p>
			</section>
	...
	<script>
		...
		function gen_short_url(){
			...
			$.ajax({ 
				method :'POST',
				url: 'Url_controller/test', 
				data: { full_url: full_url
				}, 
				// dataType: "json",
				success: function (msg) { 
					test = jQuery.parseJSON(msg)
					$('#show_short_url').show();
					$('#short_url').val(test.short_url);
					console.log(test.short_url)
				}
   			 });
		}
		...
		function gen_qr_code(){
			...
			$('#gen_qr_code').empty();
			var html = `<img src="https://chart.googleapis.com/chart?cht=qr&chs=150x150&chl=`+short_url+`" alt="">`;
			$('#gen_qr_code').append(html);
			$('#show_qr_code').show();
		}
